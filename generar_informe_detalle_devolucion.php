...
$pdf->SetAuthor('ESFIM');
$pdf->SetTitle('Detalle Devolucion');
$pdf->SetSubject('Reporte de Devolucion');
$pdf->SetKeywords('TCPDF, PDF, Devoluciones');
...
if ($id_devolucion > 0) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $html = '<h2 style="color: #007bff;">Detalles de la Devolucion</h2>';  // Título azul

        // Mostrar el Código de Asignación y Nombre de Usuario sin usar tablas
        $html .= '<p><strong>Código de Devolucion: </strong>' . htmlspecialchars($row['id']) . '</p>';
        ...
        $html .= '<h3 style="color: #007bff;">Equipos Regresados</h3>';  // Subtítulo azul
        ...
        $stmt_equipos = $mysqli->prepare("SELECT Equipo, placa_equipo, Cantidad_entregada FROM historial_entregas WHERE id_entrega = ?");
        $stmt_equipos->bind_param("i", $id_devolucion);
        ...
        if ($result_equipos->num_rows > 0) {
            while ($equipo = $result_equipos->fetch_assoc()) {
                ...
                $html .= '<td>' . htmlspecialchars($equipo['Cantidad_entregada']) . '</td>';
                $html .= '</tr>';
            }
        } else {
            $html .= '<tr><td colspan="3">No se encontraron equipos asignados para esta devolucion.</td></tr>';
        }

        $html .= '</tbody>';
        $html .= '</table>';
    } else {
        $html = '<h2>Error</h2><p>No se encontraron registros para esta dvolucion.</p>';
    }
    ...
    $html = '<h2>Error</h2><p>Devolucion no válida.</p>';
    $pdf->writeHTML($html, true, false, true, false, '');
}
...
$pdf->Output('Reporte_detalle_devolucion.pdf', 'I');
