...
$pdf->SetAuthor('ESFIM');
$pdf->SetTitle('Informe Asignaciones');
...
$html = '<h1 style="font-family: Nunito, sans-serif;">Informe General de Asignaciones</h1>';
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $html .= '<tr>';
        $html .= '<td>' . $row['id_asignacion'] . '</td>';
        $html .= '<td>' . $row['Nombre_usuario'] . '</td>';
        $html .= '<td>' . $row['Fecha_asignacion'] . '</td>';
...
$pdf->Output('Reporte_general_asignaciones.pdf', 'I');
